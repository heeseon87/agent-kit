...
# How to render — Anthropic long-form essay theme
...
This style is for scroll-worthy narrative explanations — when the topic deserves a magazine-length treatment with full literary affordances (drop caps, footnotes, large headings, pull quotes, ornaments). For a lighter blog-tone HTML page, use `/claude-kit:explain`. For a code-specific walkthrough with sidebar + Key files + Gotchas, use `/claude-kit:explain-code`.
